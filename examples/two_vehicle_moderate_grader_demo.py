...
def run_two_vehicle_episode(cfg: ColdChainConfig, seed: int, vehicle_specs: list[dict], max_steps: int = 40):
    ...
    while not done and step < max_steps:
        ...
        shipment_temps_before = {
            idx: float(env.shipments[idx].cargo_temp)
            for idx in range(len(vehicle_specs))
        }
        ...
        for idx in range(len(vehicle_specs)):
            ...
            if delivery_events[idx] is None and shipment.is_delivered:
                delivery_events[idx] = {
                    "step": step,
                    "temp_before_step": shipment_temps_before[idx],
                    "temp_after_delivery": float(shipment.cargo_temp),
                    "location": int(env.vehicles[vehicle_specs[idx]["vehicle_index"]].location),
                }
    ...
    score = ModerateGrader(trajectory).score()
    result = {
        "seed": seed,
        "paths": paths,
...
def main() -> int:
    ...
    output_root = "outputs/grader_results/moderate"
    draw_multi_vehicle_path_png(
        graph_env,
        [result["compact_paths"][0], result["compact_paths"][1]],
        VEHICLE_SPECS,
        f"{output_root}/two_vehicle_moderate_model_paths.png",
        "Two-vehicle shortest paths (ModerateGrader)",
    )
    graph_env.close()

    os.makedirs(output_root, exist_ok=True)
    report_path = f"{output_root}/two_vehicle_moderate_grader.txt"
    with open(report_path, "w", encoding="utf-8") as handle:
        handle.write("Two-vehicle ModerateGrader Demo\n")
        handle.write("==============================\n\n")
        handle.write(f"Seed used: {seed}\n")
        handle.write(f"Hard temp margin: {cfg.temp_integrity_hard_margin:.3f}\n")
        handle.write(f"Borderline temp margin: {cfg.temp_integrity_borderline_margin:.3f}\n\n")
        for spec in VEHICLE_SPECS:
            idx = int(spec["vehicle_index"])
            handle.write(f"Vehicle {idx}: start {spec['start_node']} -> target {spec['target_node']}\n")
        handle.write(f"ModerateGrader: {result['moderate_score']:.4f}\n\n")
        for spec in VEHICLE_SPECS:
            idx = int(spec["vehicle_index"])
            ...
            event = result["delivery_events"][idx]
            if event is not None:
                handle.write(
                    f"Vehicle {idx} delivery step: {event['step']} | temp before step: {event['temp_before_step']:.3f} | temp after delivery: {event['temp_after_delivery']:.3f}\n"
                )
            handle.write(f"Delivered: {result['delivered'][idx]} | Destroyed: {result['destroyed'][idx]} | Final temp: {result['cargo_temps'][idx]:.3f}\n\n")

    print("=== TWO-VEHICLE MODERATE GRADER DEMO ===")
    print(f"Seed used: {seed}")
    print(f"Hard temp margin: {cfg.temp_integrity_hard_margin:.3f}")
    print(f"Borderline temp margin: {cfg.temp_integrity_borderline_margin:.3f}")
    for spec in VEHICLE_SPECS:
        idx = int(spec["vehicle_index"])
    ...
    print(f"ModerateGrader: {result['moderate_score']:.4f}")
    print(f"Saved graph PNG: {output_root}/two_vehicle_moderate_model_paths.png")
